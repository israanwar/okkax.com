...
export default function EventStudio() {
  ...
  const initialEventId = searchParams.get("event_id") || localStorage.getItem("okkax_active_studio_event_id") || "";
  const initialDomain = searchParams.get("domain") || "EVENT";
  const initialSubview = searchParams.get("view") || "brief";
  ...
  const [saveStatus, setSaveStatus] = useState("Saved"); // "Saved" | "Saving..." | "Save failed · Retry"
  ...
  // 2. Fetch Active Event Context Data
  const loadActiveEvent = useCallback(async (eventId) => {
    if (!eventId) {
      setEvent(null);
      setLoading(false);
      return;
    ...
    setLoading(true);
    try {
      const [{ data: bData }, { data: bgData }, { data: bpData }, { data: reqData }] = await Promise.all([
        api.get(`/events/${eventId}/brief`),
        api.get(`/events/${eventId}/budget`).catch(() => ({ data: null })),
        api.get(`/events/${eventId}/blueprint`).catch(() => ({ data: { blueprint: null } })),
        api.get(`/events/${eventId}/requirements`).catch(() => ({ data: { items: [] } })),
      ]);
      setEvent(bData.event);
      setBrief(bData.brief?.payload || bData.event);
      setBudget(bgData);
  ...
  useEffect(() => {
    if (selectedEventId) {
      loadActiveEvent(selectedEventId);
    }
  }, [selectedEventId, version, loadActiveEvent]);

  // URL Sync
  const switchDomain = (newDomain) => {
    setDomain(newDomain);
    const defaultSub = newDomain === "EVENT" ? "brief" : newDomain === "NETWORK" ? "talent" : "schedule";
    setSubview(defaultSub);
    setSearchParams({ event_id: selectedEventId, domain: newDomain, view: defaultSub });
  };

  const switchSubview = (newSubview) => {
    setSubview(newSubview);
    setSearchParams({ event_id: selectedEventId, domain, view: newSubview });
  };
  ...
  const selectEvent = (eventId) => {
    if (eventId === "__NEW_EVENT__") {
      ...
      setDomain("EVENT");
      setSubview("brief");
      setSearchParams({ domain: "EVENT", view: "brief" });
      return;
    }
  ...
  const handleAddRequirement = async (e) => {
    ...
    try {
      const { data } = await api.post(`/events/${selectedEventId}/requirements`, reqForm);
      setRequirements((prev) => [data.item, ...prev]);
  ...
  const handleUpdateReqStatus = async (reqId, nextStatus) => {
    try {
      const { data } = await api.patch(`/events/${selectedEventId}/requirements/${reqId}`, { status: nextStatus });
      setRequirements((prev) => prev.map((r) => (r.id === reqId ? data.item : r)));
  ...
  const sharedProps = { eventId: selectedEventId, event, onChange: () => setVersion((v) => v + 1) };
  ...
  const eventDropdownOptions = [
    ...
    ...eventsList.map((e) => ({
      value: e.id,
      label: `${e.name} (${e.event_code || e.city})`,
    })),
  ];
  ...
  return (
    <div className="okx-event-studio space-y-3 pb-16 font-gemini" data-testid="event-studio-workspace">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap items-center gap-2 min-w-0">
            ...
            <div className="w-[220px] sm:w-[260px] md:w-[300px]">
              <OkxDropdown
                value={selectedEventId || "__NEW_EVENT__"}
            ...
            <span
              ...
              className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9.5px] font-gemini-mono ${
                saveStatus === "Saved"
                  ? "text-zinc-400"
                  : saveStatus === "Saving..."
                  ? "text-white font-bold animate-pulse"
                  : "text-zinc-300 underline cursor-pointer"
              }`}
              onClick={() => saveStatus.includes("Retry") && handleBriefChange({})}
            >
              <span className={`h-1.5 w-1.5 rounded-full ${saveStatus === "Saved" ? "bg-white/40" : "bg-white"}`} />
              {saveStatus}
            </span>
          </div>
        ...
        >
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <button
              ...
              disabled={aiBusy || !selectedEventId}
              className="flex flex-col items-start gap-1 rounded-xl border border-white/[0.1] bg-white/[0.03] p-3 text-left hover:border-white/30 hover:bg-white/[0.06] transition-all cursor-pointer disabled:opacity-50"
            >
              <div className="flex items-center gap-1.5 text-xs font-bold text-white">
                ...
                <div className="space-y-1">
                  {aiResult.actions.map((a) => (
                    <div
                      key={a.step}
                      onClick={() => {
                        switchDomain(a.domain);
                        switchSubview(a.target_tab);
      ...
      {/* ============================================================
         ...
         ============================================================ */}
      {domain === "EVENT" && (
        <section className="space-y-4" data-testid="studio-domain-event-content">
          {/* Sub-view 1: Brief */}
          {subview === "brief" && (
            <div className="space-y-4" data-testid="studio-brief-view">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4 sm:p-4.5 shadow-sm">
                <div>
                  <h2 className="text-sm font-bold text-white md:text-base tracking-tight">
                    {selectedEventId ? "Parameter Event Brief" : "Buat Event Baru — Parameter Brief"}
                  </h2>
              ...
              <div className="grid gap-3.5 sm:grid-cols-2 rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4 sm:p-5 shadow-sm">
                <label className="block sm:col-span-2">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Nama Event *</span>
                  <input
                    type="text"
                    ...
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white placeholder:text-zinc-500 outline-none transition-all focus:border-white/40 focus:ring-1 focus:ring-white/20"
                  />
                </label>

                <label className="block">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Jenis Event</span>
                  <select
                    data-testid="brief-type-input"
                    value={brief?.event_type || "Konser"}
                    onChange={(e) => handleBriefChange({ event_type: e.target.value })}
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white outline-none transition-all focus:border-white/40 cursor-pointer"
                  >
                    {EVENT_TYPES.map((t) => (
                ...
                <label className="block">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Kota / Wilayah</span>
                  <input
                    type="text"
                    data-testid="brief-city-input"
                    value={brief?.city || "Makassar"}
                    onChange={(e) => handleBriefChange({ city: e.target.value })}
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white placeholder:text-zinc-500 outline-none transition-all focus:border-white/40"
                  />
                </label>

                <label className="block">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Tanggal Mulai</span>
                  <input
                    type="date"
                    ...
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white outline-none transition-all focus:border-white/40"
                  />
                </label>

                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Durasi (Hari)</span>
                    <input
                      type="number"
                      min={1}
                      value={brief?.days || 1}
                      onChange={(e) => handleBriefChange({ days: Number(e.target.value) })}
                      className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white outline-none"
                    />
                  </label>
                  <label className="block">
                    <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Setup Days</span>
                    <input
                      type="number"
                      min={1}
                ...
                <label className="block">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Target Kapasitas (Pax)</span>
                  <input
                    type="number"
                    step={100}
                    value={brief?.capacity || 2000}
                    onChange={(e) => handleBriefChange({ capacity: Number(e.target.value) })}
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white outline-none font-gemini-mono"
                  />
                </label>

                <label className="block">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Alokasi Anggaran (IDR)</span>
                  <input
                    type="number"
                    step={10000000}
                ...
                <label className="block sm:col-span-2">
                  <span className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Deskripsi & Tujuan Event</span>
                  <textarea
                    rows={3}
                    ...
                    className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-xs sm:text-[13px] text-white placeholder:text-zinc-500 outline-none"
                  />
                </label>
              </div>
            </div>
          ...
          {subview === "identity" && (
            <div className="space-y-4" data-testid="studio-identity-view">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <div key={lbl} className="rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-3.5">
                    <div className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">{lbl}</div>
          ...
          {/* Sub-view 3: Audience */}
          {subview === "audience" && (
            <div className="space-y-4" data-testid="studio-audience-view">
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4">
                  <div className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Total Kapasitas Venue</div>
                  <div className="num mt-1 text-2xl font-bold text-white font-gemini-mono">{num(event?.capacity || 2000)}</div>
                  <div className="text-[10px] text-zinc-500 mt-1">Pax maksimum</div>
                </div>
                <div className="rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4">
                  <div className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Target Fill Rate</div>
                  <div className="num mt-1 text-2xl font-bold text-white font-gemini-mono">85%</div>
                  <div className="text-[10px] text-zinc-400 mt-1 font-gemini-mono">Estimasi: {num((event?.capacity || 2000) * 0.85)} pengunjung</div>
                </div>
                <div className="rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4">
                  <div className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Segmentasi Usia</div>
                  <div className="num mt-1 text-lg font-bold text-white font-gemini-mono">{event?.age_limit || "18-35 Tahun"}</div>
                  <div className="text-[10px] text-zinc-400 mt-1">Gen-Z & Millennial Focus</div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4 sm:p-5 space-y-3">
                ...
                <div className="grid gap-2 sm:grid-cols-3 text-xs">
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                    ...
                    <div className="text-[10.5px] text-zinc-400 mt-0.5">Kuota: {num(Math.max(200, Math.floor((event?.capacity || 2000) * 0.2)))} tiket</div>
                  </div>
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                    ...
                    <div className="text-[10.5px] text-zinc-400 mt-0.5">Kuota: {num(Math.max(500, Math.floor((event?.capacity || 2000) * 0.6)))} tiket</div>
                  </div>
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                    ...
                    <div className="text-[10.5px] text-zinc-400 mt-0.5">Kuota: {num(Math.max(100, Math.floor((event?.capacity || 2000) * 0.1)))} tiket</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {subview === "budget" && (
            <div className="space-y-4" data-testid="studio-budget-view">
              <BudgetTab key={version} {...sharedProps} />
            </div>
          )}
          ...
          {subview === "requirements" && (
            <div className="space-y-3" data-testid="studio-requirements-view">
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-white/[0.08] bg-[#0c0c12]/90 backdrop-blur-xl p-4 sm:p-4.5 shadow-sm">
        ...
        <section className="space-y-4" data-testid="studio-domain-calendar-content">
          ...
          {subview === "timeline" && (
            <div className="space-y-4" data-testid="studio-calendar-timeline">
              <OperationsTab key={version} {...sharedProps} />
            </div>
          )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-xl border border-white/[0.1] bg-white/[0.02] p-3.5 flex items-start justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        switchDomain("CALENDAR");
                        switchSubview("schedule");
                      }}
                      className="inline-flex items-center gap-1 rounded-lg border border-white/20 bg-white/10 px-2.5 py-1 text-xs font-bold text-white hover:bg-white/20 transition-all cursor-pointer shrink-0"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-150">
          <div className="w-full max-w-lg rounded-2xl border border-white/[0.15] bg-[#0c0c14] p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/[0.08] pb-3">
              ...
              <button onClick={() => setReqModalOpen(false)} className="rounded p-1 text-zinc-400 hover:text-white">
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleAddRequirement} className="space-y-3 text-xs">
              <div>
                <label className="block text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Kategori</label>
                <select
                  value={reqForm.category}
                  onChange={(e) => setReqForm({ ...reqForm, category: e.target.value })}
                  className="mt-1 w-full rounded-xl border border-white/[0.12] bg-[#09090e] px-3 py-2 text-white outline-none"
                >
              ...
              <div>
                <label className="block text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Judul Requirement *</label>
                <input
                  type="text"
                  required
                  value={reqForm.title}
              ...
              <div>
                <label className="block text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Deskripsi & Spesifikasi</label>
                <textarea
                  rows={2}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Prioritas</label>
                  <select
                ...
                <div>
                  <label className="block text-[9.5px] font-bold uppercase tracking-wider text-zinc-400 font-gemini-mono">Estimasi Anggaran (IDR)</label>
                  <input
                    type="number"
                    step={1000000}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-white/[0.08]">
                <button
                  ...
                  className="rounded-xl border border-white/[0.12] px-3 py-1.5 text-xs text-zinc-400 hover:text-white"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="rounded-xl bg-white px-4 py-1.5 text-xs font-bold text-black hover:bg-zinc-200"
                >
                  Simpan Requirement
                </button>
              </div>
