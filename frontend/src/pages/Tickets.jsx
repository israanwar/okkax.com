...
export function MyTickets() {
  ...
  if (!items) return <div className="p-6 text-sm text-zinc-500">Memuat tiket…</div>;
  if (items.length === 0)
    return (
      <div data-testid="mytickets-empty" className="border border-[var(--okx-border)] bg-[var(--okx-surface)] p-10 text-center">
        <TicketIcon className="mx-auto text-zinc-600" />
        <p className="mt-3 text-sm text-zinc-400">Belum ada tiket. Jelajahi OKKAX Discover untuk membeli tiket.</p>
      </div>
    );
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" data-testid="mytickets-list">
      {items.map((t) => (
        <div key={t.id} className="border border-[var(--okx-border)] bg-[var(--okx-surface)] p-5" data-testid={`ticket-card-${t.ticket_number}`}>
          ...
          <div className="mt-4 flex items-center gap-4">
            ...
            <div className="min-w-0 text-xs text-zinc-400">
              <div className="num font-semibold text-white">{t.ticket_number}</div>
              <div className="num mt-1">{t.event?.start_date} · {t.event?.start_time}</div>
              <div className="mt-1">{t.event?.venue_name}, {t.event?.city}</div>
              <div className="mt-1 text-zinc-500">{t.event?.gate_info}</div>
              <div className="num mt-2 break-all text-[10px] text-zinc-600">{t.qr_code}</div>
            </div>
          </div>
        </div>
      ))}
