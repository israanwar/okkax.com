...
from core import db, hash_password, now_iso, nid, ROLE_KEYS
...
COLLECTIONS = [
    "users", "organizations", "talents", "venues", "vendors", "workers", "events", "event_briefs",
    "event_blueprints", "event_talents", "rider_items", "event_venues", "event_vendors", "event_jobs",
...
async def seed(force: bool = False):
    ...
    await wipe()
    pwd = os.environ.get("DEMO_PASSWORD", "Okkax#2026")
    admin_email = os.environ.get("ADMIN_EMAIL", "admin@okkax.id")
    admin_pwd = os.environ.get("ADMIN_PASSWORD", pwd)

    for oid, name, otype, city, verified in ORGS:
        await db.organizations.insert_one(dict(
    ...
    await db.users.insert_one(dict(
        ...
        terms_accepted=True, onboarded=True, created_at=now_iso()))
    ...
    for t in TALENTS:
        ...
        await db.talents.insert_one({**t, "rider_template": rider, "currency": "IDR",
                                     "availability": ["2026-08-14", "2026-08-15", "2026-08-16", "2026-09-05"],
                                     "created_at": now_iso()})
    for v in VENUES:
        await db.venues.insert_one({**v, "availability": ["2026-08-13", "2026-08-14", "2026-08-15"],
                                    "cancellation_policy": "Pembatalan H-30: deposit non-refundable",
                                    "created_at": now_iso()})
    for vid, name, cat, pmin, pmax, score in VENDORS:
        await db.vendors.insert_one(dict(
    ...
    for wid, name, cat, rate, skills, verified in WORKERS:
        await db.workers.insert_one(dict(
            id=wid, name=name, category=cat, city="Makassar", rate_per_day=rate, skills=skills,
            verified=verified, attendance_rate=95 if verified else 84, completed_events=(hash(wid) % 20) + 3,
...
async def seed_demo_event():
    ...
    await db.events.insert_one(dict(
    ...
    await db.event_briefs.insert_one(dict(id=nid(), event_id=EVENT_ID, payload=brief, created_at=now_iso()))
    ...
    for r in talent["rider_template"]:
        ...
        await db.rider_items.insert_one(dict(
            id=nid(), event_id=EVENT_ID, event_talent_id=et_id, category=r["category"], item=r["item"],
            ...
            estimated_cost=r["estimated_cost"], actual_cost=None, notes="", created_at=now_iso()))
    fee = talent["base_fee"]
    tax = int(fee * 0.02)
    await db.event_talents.insert_one(dict(
        ...
        status="Confirmed", performance_slot="2026-08-15 21:00", created_at=now_iso()))
    await db.event_talents.insert_one(dict(
    ...
    for vid, cost in vendor_plan:
        ...
        await db.event_vendors.insert_one(dict(
            id=nid(), event_id=EVENT_ID, vendor_id=vid, vendor_name=v["name"], category=v["category"],
            cost=cost, status="Confirmed" if cost > 40000000 else "Pending", created_at=now_iso()))
    ...
    for wid in ["wkr-1", "wkr-2", "wkr-3", "wkr-4", "wkr-5", "wkr-7", "wkr-12"]:
        ...
        await db.event_workers.insert_one(dict(
            id=nid(), event_id=EVENT_ID, worker_id=wid, worker_name=w["name"], role=w["category"],
            status="Selected", check_in_at=None, payment_status="Scheduled", created_at=now_iso()))
    ...
    await db.sponsor_interests.insert_one(dict(
    ...
    await db.sponsor_commitments.insert_one(dict(
        ...
        status="Confirmed", fulfillment_status="In Progress", created_at=now_iso()))
    ...
    for zid, name, cat, area, slots, price, watt in zones:
        await db.tenant_zones.insert_one(dict(
            ...
            operating_hours="15:00 - 23:00", description=f"Zona {name} pada demo OKKAX.", created_at=now_iso()))
    ...
    booths = await db.booth_slots.find({"event_id": EVENT_ID}).to_list(100)
    for b in booths[:8]:
        await db.booth_slots.update_one({"id": b["id"]}, {"$set": {"status": "occupied", "tenant_name": "Tenant Demo UMKM"}})
        await db.tenant_applications.insert_one(dict(
            id=nid(), event_id=EVENT_ID, booth_id=b["id"], booth_code=b["code"], zone_name=b["zone_name"],
    ...
    for cat, label, amt, state in extra_budget:
        await db.budget_items.insert_one(dict(
            id=nid(), event_id=EVENT_ID, category=cat, label=label, amount=amt, state=state,
            source="manual", created_at=now_iso()))
    await db.funding_items.insert_one(dict(
        id=nid(), event_id=EVENT_ID, source="Organizer Budget", label="Aruna marketing budget",
        amount=1500000000, state="committed", created_at=now_iso()))
    await db.funding_items.insert_one(dict(
        id=nid(), event_id=EVENT_ID, source="Partner Contributions", label="Media partner barter",
        amount=80000000, state="committed", created_at=now_iso()))
    ...
    for desc, pct, due in ms:
        await db.payment_milestones.insert_one(dict(
            id=nid(), event_id=EVENT_ID, ref_type="talent", ref_id=et_id, ref_name="Aksara Band",
    ...
    for t, act, loc, owner, dep, st in rows:
        await db.schedule_items.insert_one(dict(
            id=nid(), event_id=EVENT_ID, day="2026-08-15", time=t, activity=act, location=loc,
            owner=owner, dependency=dep, status=st, notes="", created_at=now_iso()))
    ...
    for r, sev, mit in risks:
        await db.risks.insert_one(dict(id=nid(), event_id=EVENT_ID, risk=r, severity=sev, mitigation=mit,
                                      status="Open", created_at=now_iso()))
    await db.incidents.insert_one(dict(
        id=nid(), event_id=EVENT_ID, category="Production", severity="Low", time="2026-08-13 10:20",
    ...
    tax = int(gross * 0.11)
    total = gross + fee + tax
    await db.ticket_orders.insert_one(dict(
        ...
        payment_id="pay-demo-1", created_at=now_iso()))
    await db.payments.insert_one(dict(
        ...
        method_channel="BCA Virtual Account", status="Simulated Paid", created_at=now_iso(),
        due_date=now_iso(), paid_at=now_iso(), expired_at=None, refund_status=None,
        reference_number="8808123456789012",
        audit=[{"at": now_iso(), "action": "created"}, {"at": now_iso(), "action": "simulated_paid"}]))
    for i in range(qty):
        tno = f"OKX-TIX-{i+1:06d}"
        await db.tickets.insert_one(dict(
            id=nid(), ticket_number=tno, qr_code=f"OKKAX|{EVENT_CODE}|{tno}", order_id=order_id,
    ...
    await db.ticket_orders.insert_one(dict(
        ...
        payment_id="pay-demo-2", created_at=now_iso()))
    await db.payments.insert_one(dict(
        ...
        status="Refunded", created_at=now_iso(), paid_at=now_iso(), refund_status="Refunded",
        reference_number="GP-9981726354", audit=[{"at": now_iso(), "action": "refunded"}]))
    await db.refunds.insert_one(dict(
        id=nid(), payment_id="pay-demo-2", order_id="ord-demo-2", event_id=EVENT_ID,
        ...
        processed_date=now_iso(), created_at=now_iso()))

    # Blueprint from deterministic compiler
    from compiler import _fallback
    bp = _fallback(brief)
    bp["source"] = "seed_rule_based"
    await db.event_blueprints.insert_one(dict(id=nid(), event_id=EVENT_ID, **bp, created_at=now_iso()))

    await db.notifications.insert_one(dict(
        id=nid(), user_id="usr-1", event_id=EVENT_ID, title="Sponsor commitment dikonfirmasi",
        body="Sentra Digital Pay mengonfirmasi Main Sponsor Rp200.000.000.", kind="success",
        read=False, created_at=now_iso()))
    await db.notifications.insert_one(dict(
        id=nid(), user_id="usr-1", event_id=EVENT_ID, title="Funding gap perlu perhatian",
