...
def test_user_sees_events_from_every_active_membership():
    """worker@okkax.id (usr-8, no legacy org_id) becomes member of
    org A and org B. Their `GET /api/events` result must include the
    event owned by each org."""
    ...
    assert EVENT_A_ID in ids, "org A membership event missing"
    assert EVENT_B_ID in ids, "org B membership event missing"
    assert EVENT_FOREIGN_ID not in ids, "foreign-org event leaked"
